...
Fully implemented (753/753 tests). See `design/pipes-and-quirks.md`. Pipe stress test results: 10/10 scenarios expressible (7 clean, 3 with minor workarounds). Workarounds: (1) source context after collect — resolved via `pipe.withProvenance`; (2) independent filter composition across scopes — acceptable, combine predicates in one policy; (3) walk-order sensitivity for discriminators — accepted tradeoff, same as entity context today. Grammar covers: service discovery, DNS aggregation, backup orchestration with user data, mutual TLS, feature flags, multi-tier isolation, composed secret filtering, config export (oddlama pattern), conditional collection, and diamond dedup.

### stream-architecture (Den 2)

Not yet implemented. See `design/stream-architecture-migration.md` (reference analysis), `design/den2-stream-rebuild.md` (target architecture), `design/den2-incremental-delivery.md` (10-phase delivery plan). Proposes replacing the handler-based trampoline (~7,300 lines) with stream composition on Ned's primitives (~1,400 lines). Parallel-engines strategy: build `resolve2` alongside current engine, port tests incrementally, delete old engine when all 753 tests pass.
